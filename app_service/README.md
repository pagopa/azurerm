...
| <a name="input_http2_enabled"></a> [http2\_enabled](#input\_http2\_enabled) | (Optional) Is HTTP2 Enabled on this App Service? Defaults to true. | `bool` | `true` | no |
| <a name="input_https_only"></a> [https\_only](#input\_https\_only) | (Optional) Enforce https connection. Default true | `bool` | `true` | no |
...
| <a name="input_storage_mounts"></a> [storage\_mounts](#input\_storage\_mounts) | List of mapping with storage path mapping (name,type,account\_name,share\_name,access\_key,mount\_path) | `list(map(string))` | `[]` | no |
