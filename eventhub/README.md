...
| <a name="input_network_rulesets"></a> [network\_rulesets](#input\_network\_rulesets) | Define the network rule sets for the namespace | <pre>list(object({<br>    default_action = string #The default action to take when a rule is not matched. Possible values are Allow and Deny.<br>    virtual_network_rule = list(object({<br>      subnet_id                                       = string<br>      ignore_missing_virtual_network_service_endpoint = bool<br>    }))<br>    ip_rule = list(object({<br>      ip_mask = string<br>      action  = string<br>    }))<br>    trusted_service_access_enabled = bool #Whether Trusted Microsoft Services are allowed to bypass firewall.<br>  }))</pre> | `[]` | no |
